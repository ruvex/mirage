...
namespace {
    class CMIRAGEAddressVisitor : public boost::static_visitor<bool> {
    private:
        CMIRAGEAddress *addr;
    public:
        CMIRAGEAddressVisitor(CMIRAGEAddress *addrIn) : addr(addrIn) { }
...
bool CMIRAGEAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CMIRAGEAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CMIRAGEAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CMIRAGEAddressVisitor(this), dest);
}

bool CMIRAGEAddress::IsValid() const {
...
CTxDestination CMIRAGEAddress::Get() const {
    if (!IsValid())
...
bool CMIRAGEAddress::GetKeyID(CKeyID &keyID) const {
...
bool CMIRAGEAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CMIRAGESecret::SetKey(const CKey& vchSecret) {
...
CKey CMIRAGESecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CMIRAGESecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CMIRAGESecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CMIRAGESecret::SetString(const std::string& strSecret) {
