...
public:
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseBlackTheme ? QColor(180, 180, 180) : foreground);
        QRect boundingRect;
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMIRAGEAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMIRAGEAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeMIRAGEAmount = strAnonymizeMIRAGEAmount.remove(strAnonymizeMIRAGEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMIRAGEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMIRAGEAmount*COIN) nMaxToAnonymize = nAnonymizeMIRAGEAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMIRAGEAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMIRAGEAmount));
        //strAnonymizeMIRAGEAmount = strAnonymizeMIRAGEAmount.remove(strAnonymizeMIRAGEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMIRAGEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
