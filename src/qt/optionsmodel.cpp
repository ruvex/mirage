...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMIRAGEAmount"))
        settings.setValue("nAnonymizeMIRAGEAmount", 1000);
    nAnonymizeMIRAGEAmount = settings.value("nAnonymizeMIRAGEAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMIRAGEAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMIRAGEAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMIRAGEAmount:
            return QVariant(nAnonymizeMIRAGEAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMIRAGEAmount:
            nAnonymizeMIRAGEAmount = value.toInt();
            settings.setValue("nAnonymizeMIRAGEAmount", nAnonymizeMIRAGEAmount);
            emit AnonymizeMIRAGEAmountChanged(nAnonymizeMIRAGEAmount);
            break;
        default:
            break;
        }
