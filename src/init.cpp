...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MIRAGE.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MIRAGEd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMIRAGEamount=<n> " + _("Keep N MIRAGE anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MIRAGE is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MIRAGE is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MIRAGE version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MIRAGE server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MIRAGE") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MIRAGE to complete") << "\n";
    ...
    nAnonymizeMIRAGEAmount = GetArg("-anonymizeMIRAGEamount", 0);
    if(nAnonymizeMIRAGEAmount > 999999) nAnonymizeMIRAGEAmount = 999999;
    if(nAnonymizeMIRAGEAmount < 2) nAnonymizeMIRAGEAmount = 2;
    ...
    LogPrintf("Anonymize MIRAGE Amount %d\n", nAnonymizeMIRAGEAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MIRAGE+1000 == (.1MIRAGE+100)*10
       10MIRAGE+10000 == (1MIRAGE+1000)*10
    */
